...
function ReverseCartApp({ resumeId: resumeIdProp }: { resumeId?: string }) {
  ...
  const checkoutEligible = linkedWinners.every((offer) => offer?.dataSource === "liteapi" && offer.rateSource === "liteapi");
  ...
  function renderOfferCard(offer: Offer, leader: boolean) {
    return <article className={`offer-card ${leader ? "leader" : ""}`} key={offer.id}>
      ...
      <div className="hotel-head"><div className="hotel-mark" style={{ background: offer.color }}>{offer.mark}</div><div><h3>{offer.hotel}</h3><span>★ {offer.rating} · {offer.distance} km straight-line</span><small className={`source-badge ${offer.dataSource || "fixture"}`}>{offer.dataSource === "liteapi" && offer.rateSource === "liteapi" ? "LiteAPI sandbox hotel + date-specific rate" : offer.dataSource === "liteapi" ? "LiteAPI hotel · simulated rate" : offer.dataSource === "geoapify" ? "Geoapify place · simulated rate" : "Simulated fallback offer"}</small></div></div>
        ...
        <section className="content-page decision-page page-enter">
          ...
          <div className="decision-layout">
            <article className="winner-card card">
              ...
              <div className="winner-top"><div className="hotel-mark big" style={{ background: winner.color }}>{winner.mark}</div><div><span className="kicker">{selectedOfferId || selectedLegOfferIds[activeDecisionLeg] ? "YOUR SELECTION" : "RECOMMENDED"}</span><h2>{winner.hotel}</h2><p>★ {winner.rating} · {winner.distance} km straight-line from <b>{tripLegs[activeDecisionLeg]?.destination || interpretation.destination}</b></p><small className={`source-badge ${winner.dataSource || "fixture"}`}>{winner.dataSource === "liteapi" && winner.rateSource === "liteapi" ? "Hotel identity and date-specific sandbox rate from LiteAPI" : winner.dataSource === "liteapi" ? "LiteAPI hotel identity · rate simulated and payment disabled" : winner.dataSource === "geoapify" ? "Hotel identity from Geoapify · rate simulated and payment disabled" : "Fallback demo hotel · payment disabled"}</small>{winner.address && <small className="winner-address">{winner.address}</small>}</div><div className="winner-price"><small>{linkedWinners.length > 1 ? `STOP ${activeDecisionLeg + 1} TOTAL` : "FINAL TOTAL"}</small><strong>{formatINR(winner.price)}</strong></div></div>
          ...
          <div className="actions"><button className="ghost solid offer-toggle" aria-expanded={showAlternatives} onClick={() => setShowAlternatives((shown) => !shown)}>{showAlternatives ? "Hide offers ×" : linkedWinners.length > 1 ? `Show ${tripLegs[activeDecisionLeg]?.destination} alternatives +` : `Show all ${activeMarketOffers.length} offers +`}</button><button className="primary large" onClick={() => setStage("payment")} disabled={!checkoutEligible || payableTotal > interpretation.maxTotalMinor / 100}>{payableTotal > interpretation.maxTotalMinor / 100 ? "Combined total exceeds budget" : !checkoutEligible ? "Live bookable rate unavailable" : linkedWinners.length > 1 ? "Review linked trip and pay" : "Review and pay"} {checkoutEligible && <b>→</b>}</button></div>
        </section>
      )}
