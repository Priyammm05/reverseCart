...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (result.status === "completed") {
      ...
      return NextResponse.json({ status: "confirmed", transactionId: result.transactions[0]?.txn_id, bookingReference: `RC-${result.order_id}` });
    }
  ...
  async function persistStatus(status: "confirmed" | "payment_failed", transactionId?: string, bookingReference?: string) {
    ...
    await supabase.from("reservations").update({ status, booking_reference: bookingReference, confirmed_at: status === "confirmed" ? new Date().toISOString() : null }).eq("id", reservationId).eq("user_id", userId);
